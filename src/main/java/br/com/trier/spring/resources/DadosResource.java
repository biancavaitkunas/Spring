...
public class DadosResource {
	...
	public List <String> lancaDado(@PathVariable (name = "qtDados")Integer qtDados, @PathVariable (name = "aposta") Integer aposta) {
		...
		if (validaDado(qtDados)) {
			...
			for (int i = 0; i < qtDados; i++) {
				dados = (int) jogar.nextInt(1, 7);
				res.add("Valor do dado: " + String.valueOf(dados));
				somaDados += dados;	
			}
			res.add("Soma dos dados: " + String.valueOf(somaDados));
			
		} else {
			res.add("Quantidado de dados inválida! Digite um valor entre 1 e 4.");
		}
	...
	public Double percentual(Integer soma, Integer aposta) {
		Integer diferenca = Math.abs(soma - aposta);
		Double percentual = (double) diferenca / Math.max(soma, aposta) * 100;
		return percentual;
	}
